...
test('archive and restore drive Library; isolation, legacy migration and stable origins remain protected',async()=>{
 ...
 create table episodes(id uuid primary key,client_key text,template_name text,guest_name text,release_date date,created_at timestamptz default now(),archived boolean default false,completed_at timestamptz,published_at timestamptz);
