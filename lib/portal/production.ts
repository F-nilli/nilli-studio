...
const fields='id,title,format,release_date,created_at,review_url,review_ready,publication_url,published_at,status,release_at'
...
function visibleProject(p:Record<string,unknown>){return {id:p.id,title:p.title,format:p.format,release_date:p.release_date,release_at:p.release_at,status:p.status,published_at:p.published_at,review_ready:p.review_ready,review_url:p.review_ready||p.published_at?p.review_url:null,publication_url:p.published_at?p.publication_url:null}}
export async function projectFeed(clientId:string,section:string,offset=0){
 ...
 if(section==='home'){
  ...
  const upcoming=await active().gte('release_sort_at',now).order('release_sort_at',{ascending:true}).order('id').limit(1);check(upcoming.error)
  const past=await active().lt('release_sort_at',now).order('release_sort_at',{ascending:false}).order('id').limit(1);check(past.error)
