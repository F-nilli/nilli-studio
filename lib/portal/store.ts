...
import { hash, newToken, PortalError, required, staffAllowed, creatorAuthUrl, qboScope } from './core'
...
export async function issueToken(kind:string,staffId:string,accountId:string|null,seconds:number){const token=newToken();const {error}=await db().from('portal_tokens').insert({token_hash:hash(token),kind,scope:qboScope(),staff_id:staffId,account_id:accountId,expires_at:new Date(Date.now()+seconds*1000).toISOString()});check(error);return token}
export async function consumeToken(token:string,kind:string){const {data,error}=await db().from('portal_tokens').delete().eq('token_hash',hash(token)).eq('kind',kind).eq('scope',qboScope()).gt('expires_at',new Date().toISOString()).select().maybeSingle();check(error);if(!data)throw new PortalError(401,'Link expired or already used.');await validStaff(data.staff_id);return data}
export async function creator(request:Request){
 ...
 if(token.startsWith('preview_')){const {data,error}=await db().from('portal_tokens').select('staff_id,account_id').eq('token_hash',hash(token.slice(8))).eq('kind','preview_session').eq('scope',qboScope()).gt('expires_at',new Date().toISOString()).maybeSingle();check(error);if(!data)throw new PortalError(401,'Preview expired.');await validStaff(data.staff_id);accountId=data.account_id;preview=true}
 else {const url=creatorAuthUrl();const auth=createSupabase(url,required('PORTAL_AUTH_ANON_KEY'),{auth:{persistSession:false,autoRefreshToken:false}});const {data:{user},error}=await auth.auth.getUser(token);if(error||!user)throw new PortalError(401,'Session expired. Please sign in again.');const {data,error:err}=await db().from('portal_accounts').select('id').eq('creator_user_id',user.id).eq('scope',qboScope()).eq('enabled',true).maybeSingle();check(err);accountId=data?.id}
 if(!accountId)throw new PortalError(403,'Your portal account has not been enabled.');
 const {data,error}=await db().from('portal_accounts').select('*,clients(label)').eq('id',accountId).eq('scope',qboScope()).eq('enabled',true).single();check(error);if(!data)throw new PortalError(403,'Portal access is disabled.');return {account:data,preview};
}
