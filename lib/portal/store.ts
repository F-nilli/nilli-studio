...
export async function creator(request:Request){
 ...
 const {data,error}=await db().from('portal_accounts').select('id,client_id,qbo_customer_id,enabled,clients(label)').eq('id',accountId).eq('enabled',true).single();check(error);if(!data)throw new PortalError(403,'Portal access is disabled.');return {account:data,preview};
}
