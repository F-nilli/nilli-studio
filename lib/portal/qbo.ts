...
export async function syncInvoices(){return locked(async()=>{
 try {const c=await connection();const start=Date.now();const {data:accounts,error}=await db().from('portal_accounts').select('id,qbo_customer_id').eq('scope',qboScope()).eq('enabled',true).not('qbo_customer_id','is',null);check(error);let count=0
