...
## Ongoing manual billing and image attachments

Run migration_portal_manual_attachments.sql after migration_portal_manual_invoices.sql in nilli team app. Existing PDFs default to application/pdf. Manual uploads support PDF/JPEG/PNG up to 3 MB, validated using signatures rather than filenames. Edit can optionally replace the attachment; the original stays available until the new upload and metadata update succeed. Prior private attachments are retained for recovery. Clients may leave the QBO customer ID blank, save and enable their account, and use manual invoices exclusively. No live QBO authorization is needed for those accounts, but the installation still uses its existing server company/environment configuration for scope isolation. Adding a verified QBO customer later preserves that account's manual records. QBO sync ignores accounts without a QBO customer. No new environment variables.

## Independent clients and template associations

Apply `supabase/migration_portal_clients.sql` in **nilli team app** immediately before deploying the matching code. This changes the portal account client foreign key, so old creator queries using `clients(label)` cannot run after the migration until deployment completes. Schedule a brief portal maintenance window. All existing account IDs, invoices, user mappings, payment links and connections remain unchanged. Only already-configured portal accounts are copied to independent portal_clients; other production rows are not assumed to be billing clients. Template links start empty and are assigned explicitly.

Create/rename/archive clients in Client portals. One production template group (production client + template name) can belong to one portal client; clients may have many groups or none. An existing production group deletion or removal/rename of its last task clears its association without deleting the portal client or invoices. Associations do not move old episodes or expose production details to creators. Removing a link and assigning it elsewhere is explicit. Archive disables all associated account access and pauses QBO sync; restoring does not re-enable account access automatically. No delete or automatic merging of existing billing clients is provided. Independent clients are service-role-only, protected by the existing admin/ops routes. No new environment variables or cron jobs.


## Production and Library rollout

1. Run `supabase/migration_portal_production.sql` in **nilli team app**, the staff/production Supabase project. It is repeatable and does not change invoices or mark any project published.
2. Deploy the matching backend/admin PR, then publish the saved creator Site version containing Home, Production and Library.
3. In Client portals → client → Templates, confirm only that client's workflows are associated. This now grants read access to their projects. Original production identities are captured by stable ID; ambiguous/unmatched legacy keys remain hidden. Deleting and recreating a production client cannot transfer its old projects.
4. In the client's Production tab, optionally add a Frame.io review link and mark it ready for review. Confirm the actual publication date only once content is live; a published-content link is optional. Clearing publication removes it from Library.
...
Before deploying the dashboard/settings release, run `supabase/migration_portal_dashboard.sql` in **nilli team app** (the staff/production data project). It adds service-only package summaries, an optional `episodes.release_timezone` column and an appended `release_at` field on the existing service-only portal view. No task/workflow/billing records or triggers are modified. New manual project creation and release edits record the scheduler's device timezone. Existing rows with no source timezone use `workspace_settings.timezone`, then UTC; review that workspace setting for historical schedules. Date-only releases and invoice dates are never timezone shifted.

In the **Nilli Creator Portal** Supabase Auth project, keep secure email change enabled and add `https://nilli-creator-portal.nillistudio.chatgpt.site/live.html` to Authentication → URL Configuration → Redirect URLs (add the portal's actual URL if different). Email changes use Supabase's authenticated user update with confirmation, after checking the current password. No service-role auth credentials are sent to the browser. Security controls are unavailable during staff preview. Account mappings use the stable auth user ID and survive email changes. Profile and preferences use creator auth user metadata; it is never used for permissions. No auth tokens are persisted in browser storage.

Staff can edit a client's package in Client portals → client → Package, then explicitly show it in the creator portal. This is a descriptive agreement summary; it never creates charges, invoices or automated usage counts. Analytics display an honest disconnected state for the last 28 days until a separate channel analytics integration is implemented. Notification preference controls are intentionally not offered before a creator notification delivery feature exists.

Currency display supports Original, USD, CAD, BTC and sats. Original billed currency remains visible below conversions and payment links/PDFs are unchanged. The shared Next Data Cache refreshes CoinGecko BTC/fiat quotes on demand after three hours; no cron or per-user upstream calls. Fiat equivalents are indicative cross-rates via Bitcoin. Optional `COINGECKO_DEMO_API_KEY` can be set server-side if needed for provider limits; the keyless endpoint was verified during implementation. Conversion failures fall back to original amounts. Price timestamps are internal only; user-requested UI has no approximation symbols or price timestamps.
